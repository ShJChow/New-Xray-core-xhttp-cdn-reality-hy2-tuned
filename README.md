...
基于 Xray-core 的 **XHTTP + CDN + Reality + Hysteria2** 全能高可用部署方案。默认开启 **xpadding 流量填充混淆 / Hysteria2 Salamander 混淆 / 全套 7 条核心节点**，并在安装时自动应用**系统级与网络层流控调优（BBR + fq、64MB 缓冲区、1048576 句柄、全套安全加固）**，附带常驻管理工具 `xh`。

支持 V2rayN / Clash Verge Rev / Mihomo Party / Sing-box / Shadowrocket / Loon / Surge / onexray 等全平台客户端。**如果你的节点客户端内核没有更新，部分最新协议节点不可用**
...
## 十四、v4.9.7 回滚 v4.9.5 / v4.9.6 的两项改动

v4.9.5 的 64MB 缓冲上限与 v4.9.6 的 MTU 1500 **实测导致连接速度显著下降**，
本版全部回滚到 v4.9.4 的行为：

| 项 | v4.9.5/4.9.6 | v4.9.7（= v4.9.4） |
| --- | --- | --- |
| large 档 `tcp_rmem`/`tcp_wmem` 上限 | 64MB | **32MB** |
| 客户端 `tun.mtu` | 1500 | **1480** |
| 服务器网卡 MTU | 1500 | **1480** |

**教训：v4.9.5/4.9.6 的验证是不充分的。** 当时只验证了参数「是否被写进去」
（`sysctl` 回读、`ping -M do` 探通 1500 字节），**没有做任何吞吐或延迟的前后对比**。
参数写成功 ≠ 变快。MTU 能通 1500 字节的 DF 包，只证明**服务器到公网某一跳**
的路径 MTU 足够，既不代表客户端到服务器的真实路径，更不代表封装后不分片。

**回滚方法**（若你已装了 v4.9.5/4.9.6）：

```bash
# 1) 缓冲上限
xh tuning off && xh tuning on          # 装了 v4.9.7 的 xh 后重跑即可
sysctl net.ipv4.tcp_rmem               # 期望 4096 131072 33554432

# 2) 服务器网卡 MTU（若被改到 1500）
ip link set dev "$(ip route show default | awk '{print $5;exit}')" mtu 1480
grep -rn mtu /etc/netplan/             # 持久化值也要一并改回

# 3) 客户端：重新拉取订阅，或手动把配置里的 tun.mtu 改回 1480
```

> **本项目的调优参数今后不接受"没有前后对比数据"的改动。**
> 判据是同一台机、同一配置下的吞吐/延迟对比（`SAMPLES=25 python3 run_test.py`），
> 不是参数是否写入成功。
...
既然改了拿不出收益，**不改**。（sing-box 内核对该参数的处理不同，未在本版复核；**v4.9.24 已补测，结论是 sing-box 下它会把上行硬卡在 ~89 Mbps，见第二十六节**。）
...
- `policy.bufferSize: 4096`（KB）：本版**未做前后对比**，维持现状，不下结论。（**v4.9.24 已补测，见第二十六节**）
...
## 二十六、v4.9.24 三项遗留参数的实测判断

v4.9.23 留了三项没下结论。本版逐项实测后判断。工具仍是 `tools/xray_rtt_bench.py`（客户端进独立网络命名空间，
160ms RTT，按需加线路限速与丢包），新增了 sing-box 客户端支持与「上传期间并行 ping」。

### 1.〔测试环境自纠〕限速队列必须有界

本版前半程用「外层 tbf 限速 + 内层 netem 加延迟」模拟线路，内层 netem 的队列上限设成了 40 万个包。
tbf 的 `latency 100ms` 只约束它自己，管不到内层队列，**实际模拟出了 1 秒级的超大缓冲**：
上传期间并行 ping 的平均延迟 956~1092ms，而设定 RTT 只有 160ms。

在这种环境里 Brutal 会把缓冲灌满、BBR 被严重误导，得出过一个**错误结论**：
「sing-box 客户端不声明带宽（BBR）时，慢上行线路的上行崩到 8 Mbps」。
改为 netem 自带 `rate`、并按「延迟在途 + 约 100ms 排队」计算有界 `limit` 后，同一场景是 40 Mbps。
**本节以下全部数据均来自修正后的有界队列环境**，工具已同步修正（`BUF_MS` 环境变量可调排队时长）。

### 2.〔sing-box 内核〕Hysteria2 的 `upmbps=100` 会把上行硬卡在 ~89 Mbps

Xray 内核下这个参数测不出效果（见第二十五节），**但 v2rayN 的 Hysteria2 默认走 sing-box 内核**，Mihomo 也同样启用 Brutal。
sing-box 客户端实测（上行 Mbps，各 3 次中位）：

| 链接里的 `upmbps` | 线路 300↓/50↑（无丢包 / 下行 1%） | 线路 1000↓/300↑（无丢包 / 下行 1%） |
|---|---|---|
| **100（现默认）** | **41 / 42** | **85 / 85**（范围 85~85，硬天花板） |
| 300 | — / 27 | 190 / 175 |
| 1000 | 31 / 33 | 163 / 167 |
| 不声明（BBR） | 40 / 13 | 171 / 160 |

**没有哪个值两头都赢**：
- 调高后快上行翻倍，但慢上行掉 25~35%（Brutal 按声明速率硬发，超过真实线路就把自己的上行队列灌满）。
- 不声明时，「慢上行 + 丢包」下跌到 13。

Brutal 的本意是「声明值 ≈ 你的真实上行」，生成器替不了每个人猜。

**判断：默认值保持 100**（国内家宽上行多在 30~100 Mbps，100 离主流最近，改高会让多数人变慢），
**改为安装时可配置**，同时作用于 v2rayN 链接与 Mihomo 配置：

```bash
HY2_UP_MBPS=300 HY2_DOWN_MBPS=1000 bash install.sh   # 上行 ≥300 Mbps 的用户
```

已安装的用户也可以直接在客户端里把该节点的上行带宽改成自己线路的真实上行。

### 3.〔Xray 服务端〕`policy.bufferSize`：无可测差异，值不动、更正注释

先核实了源码（`features/policy/policy.go`）：arm64 上 Xray 默认确实只有 4KB。然后测了 4KB（默认，不写该字段）/ 32 / 512 / 4096 KB 四档，
每档都重启 xray，测 Reality-Vision、Reality-XHTTP、XHTTP-H3-Direct、Hysteria2，上下行各 3 次：

| bufferSize | Reality-Vision ↓ | Reality-XHTTP ↓ | XHTTP-H3 ↓ | Hysteria2 ↓ |
|---|---|---|---|---|
| 不限速 RTT 0：4KB / 32 / 512 / 4096 | 850 / 899 / 961 / 877 | 1393 / 1454 / 1347 / 1389 | 905 / 926 / 909 / 860 | 550 / 539 / 551 / 528 |
| 160ms + 1% 丢包：4KB / 32 / 512 / 4096 | 134 / 129 / 126 / 136 | 123 / 97 / 90 / 95 | 111 / 106 / 107 / 95 | 119 / 127 / 124 / 131 |

上行同样全部落在彼此范围内，xray 常驻内存 58~70MB 也无差别。

**判断：值保持 4096 不动**（改动同样拿不出收益）。生成器注释此前写着「扩容释放千兆高吞吐性能」，**与实测不符，已更正**。

### 4.〔已在 sbbox v2.7.10 落地〕Hysteria2 服务端初始窗口

这一项属于同机 sbbox 项目外置的 Hysteria2（UDP 44116），实测与决策记在 sbbox README。结论摘要：
把 QDoS 加固时收紧的初始窗口 512KB / 1MB 放大到 8MB / 20MB，快线路 40MB 上传 109→141（sing-box 客户端）、
慢线路 5MB 上传 3→30 Mbps。
- **不削弱防护**：两条 Hysteria2 都强制 salamander，实测混淆密码错误或不带混淆时连接根本建不起来。
- `ignoreClientBandwidth: true` **保持**：放开后下行没有可测收益。

本项目自己的 Hysteria2（8443，Xray 原生入站）没有配置窗口、用内核默认值，快线路上行 160~184 Mbps，不存在这个问题。

### 5.〔验证〕

- 12 节点 `SAMPLES=25 run_test.py` 回归 12/12 PASS。
- 所有 A/B 过程中改过的服务端配置均已恢复原值，只保留上面第 4 条的窗口改动。

---

## 二十七、v4.9.26 新增 Hysteria2-H3-Direct：上下行最佳、流量形态为标准 HTTP/3

### 1.〔设计〕为什么是这条

在同一套条件下（netns，160ms RTT，线路限速 + 丢包，队列有界，见第二十六节）先把现有节点排一遍：
**上下行都最好的一直是 Hysteria2**（1000↓/300↑ 线路下行 227~243、上行 168~184），其余节点都有明显短板
（Reality-XHTTP 上行被卡在 47，见下文第 3 条）。所以在同一协议上再造一条"参数更猛"的节点没有意义。

还能改进的是**网络状态**：现有 Hysteria2 跑在 UDP 8443、带 salamander 混淆，流量是随机字节的非标准端口 UDP——
这正是国内运营商最常做 QoS 限速甚至阻断的形态。新节点反过来：

- **UDP 443、不加混淆**，TLS 证书与伪装站和现有节点相同，未认证访问被转给本机站点。
- 线路上看就是一个普通网站的 HTTP/3，是 UDP 里最不容易被按端口 / 特征限速的样子。
- 认证密码与 Hysteria2-Obfs-Direct 相同，客户端只填密码、不填混淆。

### 2.〔实测〕全节点下行最快，上行与 Hysteria2 持平

下行 Mbps（每项 3 次中位；Xray 客户端 / sing-box 客户端）：

| 条件 | **Hysteria2-H3-Direct（新）** | Hysteria2-Obfs-Direct | Reality-Vision |
|---|---|---|---|
| 1000↓/300↑ 无丢包 | **257 / 263** | 227 / 206 | 198 |
| 1000↓/300↑ 下行 1% 丢包 | **159 / 132** | 126 / 108 | 128 |
| 300↓/50↑ 下行 1% 丢包 | **129 / 139** | 127 / 118 | 108 |

上行与 Hysteria2-Obfs-Direct 持平（1000↓/300↑ 无丢包：152 vs 168，1% 丢包：111 vs 196，范围互相重叠；
300↓/50↑：两者都是 44）。sing-box 客户端的上行受链接里 `upmbps` 声明约束（见第二十六节第 2 条），
上行 ≥300 的用户可用 `HY2_UP_MBPS=300` 调整。

另一个方向一致的观察：经真实域名走公网（含云厂商发夹路径）时，新节点 613 / 694 Mbps，
带混淆的 8443 只有 247 / 252。发夹路径不代表真实客户端速度，只作参考。

**验证命令**：

```bash
python3 tools/xray_compat_test.py /usr/local/nginx/html/sub/<token>/     # 订阅里应多出 Hysteria2-H3-Direct 且 PASS
ss -lnup | grep ':443 '                                                  # xray 在 UDP 443 监听
iptables -S INPUT | grep 'dport 443' | grep hashlimit                     # 握手限速规则在位
```

### 3.〔查明，无法靠配置修复〕Reality-XHTTP 的上行被 HTTP/2 窗口卡住

Reality-XHTTP 在 160ms RTT 下上行只有 ~47 Mbps，五种客户端写法（stream-up / packet-up / 缩短包间隔 / 4 连接）
结果都一样。根因在服务端：Xray 的 XHTTP 在 TCP 上用 Go 标准库 `net/http` 提供 HTTP/2，**没有设置任何 HTTP/2 流控参数**，
走 Go 默认的 1MB 上传窗口；一个会话的上行始终落在同一条 HTTP/2 连接上，多连接也救不了。
上限随 RTT 严格反比，与「1MB ÷ RTT」吻合：

| RTT | 上行实测 | 1MB 窗口理论上限 | Reality-Vision 上行（对照） |
|---|---|---|---|
| 40ms | 183 | 210 | 203 |
| 80ms | 96 | 105 | 155 |
| 160ms | 47 | 52 | 102 |

该参数配置里未开放，本项目只用官方正式版 Xray，不做源码修改。**它的下行反而比 Vision 好**（160ms 下 147 vs 121），
所以按用途选：**下载为主用 Reality-XHTTP，上传多用 Hysteria2-H3-Direct / Reality-Vision / XHTTP-H3-Direct**。
Reality-Up-CDN-Down 的上行走同一条路径，有同样的上限。

### 4.〔其他〕

- 本版同时发布此前已提交未发版的改动：Hysteria2 链接改用域名、默认不再生成 VLESS-XHTTP-CDN-H2。
- 同机 sbbox 的外置 Hysteria2 在 9-17 被有意重置为官方默认参数；本次**只加回 `ignoreClientBandwidth: true`**
