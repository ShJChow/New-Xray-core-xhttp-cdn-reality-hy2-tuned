...
> 默认关闭、按需开启：`VLESS-CDN-Up-Reality-Down`（`FEATURE_CDN_UP_REALITY_DOWN`）、`VLESS-XHTTP-CDN-H2`（`FEATURE_CDN_H2`）、`VLESS-XHTTP-Direct-H2`（`FEATURE_H2_DIRECT`）、`Hysteria2-Obfs-Direct`（`FEATURE_HY2_OBFS`）。
